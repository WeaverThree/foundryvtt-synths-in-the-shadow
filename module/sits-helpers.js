export class SitsHelpers {
  ...
  static removeDuplicatedItemType(item_data, actor) {
    let dupe_list = [];
    let distinct_types = ["unit_playbook", "unit_reputation", "playbook", "deviance", "maker", "build"];
    let allowed_types = [];
...
**/
  ...
  static async removeContact(actor, contactId){
   ...
   try{
		let new_actor = await game.actors.importFromCompendium(game.packs.get("synths-in-the-shadow.npc"),acqId);
  ...
  static async getPlaybookAcquaintances(actor_type, selected_playbook){
	...
	if (actor_type == "agent") {
		...
		playbook_acquaintances = all_acquaintances.filter(i => i.system.associated_unit_playbook === selected_playbook);
	}
